...
const SecurityLogs = () => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const data = await adminAPI.getSecurityLogs(queryParams);
      setLogs(data.logs || []);
  ...
  const confirmDeleteLog = async () => {
    ...
    try {
      await adminAPI.deleteSecurityLog(logToDelete);
      setLogToDelete(null);
  ...
  const confirmCustomDateDelete = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await adminAPI.bulkDeleteSecurityLogs(null, customStartDate, customEndDate);
      // Refresh logs after deletion
      fetchLogs();
      setCustomStartDate('');
      setCustomEndDate('');
      ...
      console.error('Error bulk deleting security logs:', err);
  ...
  return (
    <div className="w-full space-y-6 overflow-x-hidden">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div className="flex flex-col sm:flex-row gap-3 items-end">
          <div className="flex-1 w-full sm:w-auto">
            <input
              type="text"
              value={filters.userEmail || filters.ipAddress || ''}
              onChange={(e) => {
              ...
              {logs.length === 0 ? (
                ...
                <tr key={log._id} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getSeverityBadge(log.severity)}`}>
      ...
      {showCustomDateModal && (
        <ConfirmationModal
          title="Delete Security Logs by Custom Date Range"
          message={`Are you sure you want to delete all security logs from ${customStartDate} to ${customEndDate}? This action cannot be undone.`}
          confirmButtonText="Delete"
          cancelButtonText="Cancel"
